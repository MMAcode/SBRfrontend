...
import {positionElementToTheTop} from "../services/helperMethods";
import { clippingParents } from '@popperjs/core';

// height also ='auto'
const AccordionSingle = ({ children, bg = '#ffd400', titleHeight = 0, initialCardOpen = '0', cardOpenHandler }) => {
  ...
  React.useEffect(() => {
    console.log("MiraACCCCCCORDION INITIALIZED", cardNumberAsStringToShow);
  ...
  const handleTitleClick = () => {
    ...
    if (cardOpenHandler) {
        ...
        cardNumberAsStringToShowSET(before => {
            console.log(before, cardOpenHandler);
            if (before != '1') {
                return '1'
            } else {
                return '0';
